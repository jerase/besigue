// ============================================================
// ÉCRAN FIN DE MANCHE — IT-5 (SF-13.6)
// ============================================================
...
export const EcranFinManche: React.FC<EcranFinMancheProps> = ({
  ...
  const { brisques, bonusDernierPli, vainqueurManche, enBasTable,
          charlesBezigue, scoreFinJ0, scoreFinJ1 } = resultat

  const nomJ0 = config.nomJoueur1
  const nomJ1 = config.nomJoueur2
  const partieTerminee = vainqueurManche !== null
    ...
    <div className="min-h-screen bg-[#0a1628] flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg">
          ...
          {partieTerminee ? (
            <>
              ...
              <h1 className="text-3xl font-bold text-amber-300" style={{ fontFamily: 'Georgia, serif' }}>
                {vainqueurManche === 0 ? nomJ0 : nomJ1} gagne !
              </h1>
              {charlesBezigue && (
                <div className="mt-3 px-4 py-2 bg-amber-500/20 border border-amber-400/40 rounded-xl">
                  <p className="text-amber-300 font-bold text-lg">⭐ Charles Bézigue !</p>
                  <p className="text-amber-200/70 text-sm mt-1">Victoire en bas de table — l'adversaire avait moins de 750 pts</p>
                </div>
              )}
            </>
          ) : (
            <>
              ...
              <p className="text-white/50 text-sm mt-2">Aucun joueur n'a atteint 1000 points</p>
            </>
          )}
        </div>
        ...
        <p className="text-center text-white/30 text-xs mb-6">
          Seuil de victoire : 1 000 points
        </p>
...
const ValeurScore: React.FC<{ val: number; unite: string; delta?: boolean; accent?: boolean }> = ({
  ...
  const prefix = delta && val > 0 ? '+' : delta && val < 0 ? '' : ''
  const color = delta
...
const ScoreTotal: React.FC<{ score: number; estVainqueur: boolean }> = ({ score, estVainqueur }) => (
  <div className="text-center">
    ...
    {estVainqueur && <div className="text-amber-400 text-xs mt-0.5">🏆 Vainqueur</div>}
  </div>
)
